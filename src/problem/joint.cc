...
namespace roboptim
{
  namespace retargeting
  {
    namespace problem
    {
      ...
      Joint::Joint (cnoid::MarkerMotionPtr markerMotion,
	...
	: characterInfos (new std::vector<CharacterInfo> ()),
      ...
      {
	jointIMesh->addMotion (markerMotion, character);
	jointIMesh->update ();
	...
	costLaplacian_ = LaplacianDeformationEnergyShPtr_t
	  (new LaplacianDeformationEnergy
	   (animatedMesh_, markerMotion, character, jointIMesh,
	    characterInfos));

	costAcceleration_ = AccelerationEnergyShPtr_t
	  (new AccelerationEnergy (jointIMesh));
	   ...
	   frameId < markerMotion->numFrames (); ++frameId)
	{
      ...
      boneLength_ = BoneLengthShPtr_t
						       (new BoneLength
							(jointIMesh, characterInfos, numAllBones));
